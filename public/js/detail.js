...
const getCasts = function (castList) {
    ...
    for (let i = 0, len = castList.length; i < len && i < 10; i++) {
        const { name, profile_path } = castList[i];
        newCastList.push({name, profile_path});
    }
...
fetchDataFromServer(`https://api.themoviedb.org/3/movie/${movieId}?api_key=${api_key}&append_to_response=casts,videos,images,releases`, function (movie) {
    ...
    movieDetail.innerHTML = `
            ...
            <div class="detail-box">
                ...
                <div class="detail-content">
                    ...
                    <p class="overview">Synopsis : ${overview}</p>
                    ...
                    <ul class="detail-list">
                        ...
                        <div class="list-item">
                            <p class="list-name">Directer By</p>
    ...
    `;
    //nambahin foto pemeran
    const castList = getCasts(cast);
    const castListContainer = movieDetail.querySelector("#cast-list");
    castList.forEach(({ name, profile_path}) => {
        const castItem = document.createElement("div");
        castItem.classList.add("cast-item");
        castItem.innerHTML = `
        <img src="${imageBaseURL}w185${profile_path}" alt="${name}" class="cast-img">
...
const addSuggestedMovies = function({ results: movieList }, title) {
    ...
    for (const movie of movieList) {
        const movieCard = createMovieCard(movie); //di panggil dari movie_card.js
